...
//         conn.set(key.clone(), serialized).await.map_err(|e| {
//             RegistryError::Custom(format!("Failed to set key in Redis: {}", e))
//         })?;

//         if let Some(exp_time) = record.expires_at() {
//             let unix_time = exp_time
//                 .duration_since(UNIX_EPOCH)
//                 .unwrap_or_else(|_| Duration::from_secs(0))
//                 .as_secs_f64();
//             conn.zadd(self.expiration_key.clone(), key, unix_time)
//                 .await
//                 .map_err(|e| RegistryError::Custom(format!("Failed to set expiration: {}", e)))?;
//         }
...
//       let keys: Vec<String> = match conn.zrange(self.expiration_key.clone(), 0, -1).await {
//           Ok(keys) => keys,
